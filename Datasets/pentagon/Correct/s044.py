...
class Shape(Triangle,RegularPolygon):
...
class Pentagon(Triangle,Square,RegularPentagon):
  def __init__(self,s,name="Pentagon"):
    RegularPolygon.__init__(self,5,s,name)
    Triangle.__init__(self,s,s*math.sqrt(3)/2,name)

  def area(self):
    return ((self.base*self.height*.5)+(self.length*self.no_of_sides))
